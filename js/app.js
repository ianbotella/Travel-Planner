...
const ICONS = {
  ...
  experiencia: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="2.3"/><path d="M12 3v6.3M4.8 16.3l5.1-2.9M19.2 16.3l-5.1-2.9"/></svg>`,
...
/* Al navegar a un link de la checklist de pendientes/completados (#logibox-...)
   el navegador hace scroll pero no despliega el <details> colapsado por su cuenta */
function openTargetLogibox() {
...
function setupHashOpen() {
  window.addEventListener("hashchange", openTargetLogibox);
}
...
function logiboxHtml(item, kind, context) {
  ...
  return `
    <details class="logibox logibox--${kind} ${doneClass}" id="logibox-${kind}-${item.id}" data-item-id="${item.id}" data-kind="${kind}">
      <summary>
        <span class="logibox__check" aria-hidden="true">${checkMark}</span>
        <span class="logibox__icon" aria-hidden="true">${icon}</span>
